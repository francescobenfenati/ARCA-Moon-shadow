...
int scan_beta() {
 ...
 string daz, dalt, fitinf0, sundist, MC_sun_dist;
 int nHitFit;
 
 double sig = 0.5;
 
 double beta_min =  4.5;//0.1;
 double beta_max = 5.5;//1.;
 double beta_cut = beta_min;
 double beta_step = 0.1;//0.01;
...
// ---------- define 1D projection of event distribution to obtain BG Azimuth fit
 ...
 // ---------- loop over n_beta  --------------------------
   ...
   while(beta_cut<beta_max+beta_step) {
 ...
 double SigFit = sig; 
    ...
    bg2->FixParameter(3,0);
    bg2->FixParameter(4,0);
    bg2->FixParameter(5,0);
    bg2->FixParameter(6,0);
   ...
   ofstream myfile("csv/arca8/arca8rbr_mu_pre+anoise+sun_betas_5.txt",ios::app);
   if(myfile){
     myfile << beta_cut <<" " << lamb <<" "<< Sig0 <<" "<< amp0 << endl;
   }
