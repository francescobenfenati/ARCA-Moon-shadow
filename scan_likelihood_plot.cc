...
int scan_likelihood_plot() {
 
  TCanvas* c[3];
  TGraph* gr[3];

  gr[0] = new TGraph();
  gr[0]->SetTitle("Likelihood scan; like_cut ; lambda");
  ...
  gr[1] = new TGraph();
  gr[1]->SetTitle("Likelihood scan; like_cut (deg) ; sigma");
  ...
  gr[2] = new TGraph();
  gr[2]->SetTitle("Likelihood scan; like_cut (deg) ; amplitude");
 ...
 ifstream fstream;
 fstream.open("csv/arca8/arca8rbr_mu_pre+anoise+combined_shadow_likes.txt");
 {
   while (!fstream.eof()) {
	
     fstream >> lik >> lamb >> sig >> amp;
   
     double beta_cut = std::stof(lik);
