...
int main(int argc, char** argv) {
  ...
  TFile *file=new TFile("MCsun.root", "RECREATE");

  string filename, daz, dalt, fitinf0, sundist, MC_sun_dist;
  double chi;
  int nHitFit;
 
 double sig = 0.5; //0.26 ARCA6 MS 0.25 ARCA8 and ARCA
 double beta_cut = std::stof(argv[3]);//0.32;//0.32;//<---------------------------------- ARCA8 MS
 //double beta_cut = 0.31;//<---------------------------------- ARCA6 MS 
 cout<<"sig = "<<sig<<endl;
...
// ---------- define 2D histogram of event number distribution
 ...
 h2_noshad->SetXTitle("x [deg]");
 h2_noshad->SetYTitle("y [deg]");
 ...
 h2_shad->SetXTitle("x [deg]");
 h2_shad->SetYTitle("y [deg]");
...
// ---------- define 1D projection of event distribution to obtain BG Azimuth fit
 ...
 //hAzi->GetYaxis()->SetRangeUser(0,120);
 // ---------- loop over n_beta  --------------------------
 ...
 int n_h1 =0, n_h0 =0;
   ...
   //fstream.open("csv/arca8/MC/arca8rbr_mu_pre+anoise+sun.txt");
   //fstream.open("csv/arca8/MC/arca8rbr_mu_pre+anoise+moon.txt");
   fstream.open(argv[1]);
   //fstream.open("csv/arca6+8rbr_mu_pre+anoise+combined_shadow.txt");
 {
      while ( ! fstream.eof() )
    {
     fstream >> daz >> dalt >> fitinf0 >> sundist >> MC_sun_dist;
 ...
 double x1 = pol2->GetParameter(1)/a0;
 double x2 = pol2->GetParameter(2)/a0;
 double x3 = 0; //pol3->GetParameter(3)/a0;
 double chi2azi = pol2->GetChisquare(); //<------------ no need of multiplying by 2 as the minimum fcn value is already the pearson chi2 value!! 
 ...
 */
 ...
 double y3 = 0; //pol3->GetParameter(3)/a0;
...
// define 2d fit function, initialize parameters
 ...
 //hAzi->Draw("H");
 //hAzi->Draw("E, P, SAME");
 //c0->SaveAs("/sps/km3net/users/fbenfe/Moon_shadow/azimuth_distr.pdf");
 ...
 gPad->Update();
 /*
 auto stat = dynamic_cast<TPaveStats*>(hy->FindObject("stats"));
 if (stat) {
 ...
 c1->SaveAs("/sps/km3net/users/fbenfe/Moon_shadow/altitude_distr.pdf");

 */

 //--------------Plot/ 2D DISTRIBUTIONS SHADOW & NO SHADOW--------------------
 //TCanvas *c2 = new TCanvas("c2", "no_shad_distribution", 200, 10, 600, 400);
 //c2->cd();
 //h2_noshad->Draw("colz");
 //c2->SaveAs("/sps/km3net/users/fbenfe/Moon_shadow/2D_S0_distr.pdf);

 //TCanvas *c3 = new TCanvas("c3", "shad_distribution", 200, 10, 600, 400);
 //c3->cd();
 //h2_shad->Draw("colz");
 //c3->SaveAs("/sps/km3net/users/fbenfe/Moon_shadow/2D_S1_distr.pdf");
 ...
 Double_t f2params[npar] = {a0,AziFit0,AltFit0,SigFit,amp,y1,y2,y3,x1,x2,x3};
    ...
    const Int_t np_bkg = 7;
    Double_t b2params[np_bkg] = {a0,y1,y2,y3,x1,x2,x3};
    TF2 *bkg2 = new TF2("bkg2",bkg,-MaxGrad,MaxGrad,-MaxGrad,MaxGrad, np_bkg);
   ...
   // ---------- 2Dim Fit on h2 histogram of event distribution
    ...
    h2_shad->Fit("f2","NL"); // "B" option if fixed Sigma !
   ...
   cout << "Events selected in H0 : "<<n_h0<<" and in H1 : "<<n_h1<<endl;
   cout << "--------- BEST SHADOW POSITION FIT (XS, YS) --------  "<<endl;
   cout << "Sigma resolution: "<< f2->GetParameter(3)<<" +- "<<f2->GetParError(3)<<endl;
   cout << "amp = " << amp <<" +- "<<f2->GetParError(4)<< endl;
   cout << "Chi2 (best fit 0.1 deg bin, +-6 grad) = " << chi2Fit << endl;
   cout << "XS = " << XS << endl;
   cout << "YS =  " << YS << endl;
   ...
   cout << "sigma = "<<sqrt(2)*TMath::ErfInverse(1.-2*TMath::Prob(-lambda_0,2)) << endl; // 2 becomes 1 if for nominal fit sigma is fixed   
   cout << "p-value(theta) = " << TMath::Prob(theta,2) <<endl;// 2 becomes 1 if for nominal fit sigma is fixed
   cout << "sigma = "<<sqrt(2)*TMath::ErfInverse(1.-2*TMath::Prob(theta,2)) << endl; // 2 becomes 1 if for nominal fit sigma is fixed
...
// --------------- fill the delta chi2 histogram from refitting amplitude
 ...
 for (int i=0;i<NbinChi2;i++) { // loop over x-axis in chi2 histogram
   for (int j=0;j<NbinChi2;j++) { // loop over y-axis in chi2 histogram
     ...
     f2->SetParameter(6,A2);
     f2->FixParameter(7,A3);
     f2->FixParameter(8,B1);
     f2->FixParameter(9,B2);
     f2->FixParameter(10,B3);
 ...
 std::string hist_name = std::string("/sps/km3net/users/fbenfe/Moon_shadow/plot/")+std::string(argv[2])+std::string("_bin_")+std::string(argv[4])+std::string("_2d_histogram.pdf");
 const char* hist_name_char = hist_name.c_str();
 std::string contour_name = std::string("/sps/km3net/users/fbenfe/Moon_shadow/plot/")+std::string(argv[2])+std::string("_bin_")+std::string(argv[4])+std::string("_contour.pdf");
